...
本项目借鉴了 `ycarowr/UiCard` 的卡牌界面配置思路，提供手牌间距、卡牌悬停缩放、抬起高度、旋转角度和禁用透明度的可调参数。你可以在战斗界面底部打开“UI 配置”面板，实时调整这些交互效果。
...
- 鼠标悬停时卡牌会自动抬起，表示选中状态
- 按下卡牌后即可拖拽
- 敌人以卡片形式展示，可将手牌拖动到敌人目标上进行交互
- 拖动过程中会显示一条箭头，表示当前卡牌的出发方向与应用目标
